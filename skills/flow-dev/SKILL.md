---
name: flow-dev
description: lionad 的开发流程，将零散的需求任务重新组织成拆解、TDD Dev、Review、验证到维护记忆、更新文档的一整条的成熟工业流程。
argument-hint: <task description> [--mode light|full|quick|dev|fix] [--depth mvp|prod|hifi] [--dissection] [--worktree] [--skip-review] [--interactive] [--dry-run] [--stage] [--stop dissection|thinking|prd|devgoal|slice] [--resume [task-slug]]
metadata:
...
一句话记忆：**模式选 mode，质量选 depth，拆解加 dissection，断点 stop/resume，其余都是微调**。
...
- `<caller-model>`（当前主代理模型）：根据 Claude Code 实际使用的模型标识推断：`kimi` → `kimi-k3`；`glm` → `glm-5.3`；`qwen` → `qwen-3.8-max`；`deepseek` → `deepseek-v4-flash`；`minimax` → `minimax-m3`；默认 `glm-5.3`。合法枚举以 run-external-review 的 `--caller-model` choices 为唯一真源（本表是快照），漂移时按其 exit 2 报错的合法枚举修正并同步本表
...
`--mode dev` 下 `dissection`、`thinking`、`prd` 三个停止点不可用（对应阶段已跳过），Step 0 校验时报错并停止；仅 `devgoal`、`slice` 有效。`--mode quick` 下 `dissection`（与 quick 互斥）、`prd`（无 PRD 阶段）不可用；`thinking`、`devgoal`、`slice` 有效。`--mode fix` 同 dev：`dissection`、`thinking`、`prd` 不可用；`devgoal`、`slice` 有效。
...
0. 初始化上下文
   ...
   - [ ] **使用 Task 工具创建任务列表**，模板（列表顺序即执行顺序）：
     ...
     - `Step 4-7 S<i>: <Slice 名> DevLoop(开发→审查→修复→提交)`——每个 Slice 一条，按 DevGoal 的有序 Slice 列表展开；Slice 名凝练自该 Slice 目标并带关键对象（如 `Step 4-7 S1: 泄漏修复（mount/visibleOnHover）DevLoop(开发→审查→修复→提交)`）
...
1. 从用户输入或上下文，捕获我想构建的内容的原始想法，对原始想法进行**极其细致**的分析，扩充成 UltraThoughts
   ...
   - [ ] **若 `--dissection`**：执行需求树拆解（完整纪律见 `references/dissection.md`），拆解产物落档到独立文档（不并入 UltraThoughts）：
     ...
     - [ ] **外部审查检查点（需求树拆解，Gate）**：若未传 `--skip-review`，对拆解结果做正交审查，未经检查不得进入下游（审查纪律见 `references/dissection.md`「外部正交审查」；审查 prompt 按陈述式：陈述需求树的规模与覆盖边界——中间节点数、叶节点数、自我批评三问的裁决结果；不下达"重点关注 X"的审查指令）：
       ...
       - [ ] 调用 `external-review <working-dir>/docs/dissections/<task-slug>.md --task "<caller-model> 完成需求树拆解" --slug <task-slug>-dissection --caller-model <caller-model> --effort normal --target-model auto --output-dir <working-dir>/docs/reviews`
   ...
   - [ ] **外部审查检查点（UltraThoughts）**：若 `--mode full` 且未传 `--skip-review`（审查 prompt 按陈述式：陈述 UltraThoughts 已产出的目标定义性属性与可证伪验证，审查模型据此判断属性是否准确、验证是否真能证伪；不下达"重点关注 X"的审查指令）：
     ...
     - [ ] 调用 `external-review <working-dir>/docs/thoughts/<task-slug>.md --task "<caller-model> 完成 UltraThoughts 需求分析" --slug <task-slug>-ultrathoughts --caller-model <caller-model> --effort normal --target-model auto --output-dir <working-dir>/docs/reviews`
     - [ ] 将返回的 JSON 回填到该 review 条目的 `reviews[]`，并设置 `end_time = node -e "console.log(new Date().toLocaleString('sv-SE'))"`
...
2. 针对 UltraThoughts 执行 `grill-me` 技能；**仅 `--mode full`** 时在问询结束后使用 `to-prd` 技能把 PRD 文档沉淀到 `<working-dir>/docs/plans/<task-slug>.md`（`--mode light` 跳过 `to-prd`，不生成 PRD）
   ...
   - [ ] **外部审查检查点（grill-me）**：若 `--mode full` 且未传 `--skip-review`：
     ...
     - [ ] 调用 `external-review <run-dir>/decisions.md --task "<caller-model> 完成 grill-me 自问自答" --slug <task-slug>-grill-me --caller-model <caller-model> --effort normal --target-model auto --output-dir <working-dir>/docs/reviews`
     - [ ] 将返回的 JSON 回填到该 review 条目的 `reviews[]`，并设置 `end_time = node -e "console.log(new Date().toLocaleString('sv-SE'))"`
     - [ ] 若任一模型 `status=degraded`，把降级原因写入决策台账
...
4. 自动确认 DevGoal，按顺序进入每个 Slice 的 开发 → 审查 → 修复 循环
   ...
   - [ ] 已自动确认 DevGoal
   - [ ] **并行窗口（可选）**：无依赖且文件互不重叠的 Slice，可用子代理并行开发（分派 prompt 写清目标、文件范围与验收命令，子代理不执行 git 操作——并行 Slice 的提交统一由主代理在各 Slice 收口时按各自文件清单串行执行）
   - [ ] 对每个 Slice 执行以下子流程：
     - [ ] 声明当前 Slice 名称与目标
     ...
     - [ ] 进入开发，直到该 Slice 的 DevGoal 达成；**开发中允许子任务提交**（`--stage` 时除外）：完成一个内聚子任务即可提交（仅 `git add` 该子任务涉及的文件），提交纪律同 Step 7 收口项；子任务提交非必须，Slice 收尾统一收口见 Step 7
   ...
   - [ ] **外部审查提前后台发起**（满足 Step 6 触发判据时执行；未触发或传入 `--skip-review` 时跳过本项，由 Step 6 记 skipped；`--mode fix` 恒跳过本项，Step 6 按模式禁用记 skipped）：本地 code-review 与外部审查互不依赖——prompt.md 只陈述已查维度、不含本地审查结果，提前发起让两者并行
     ...
     - [ ] 后台调用 runner（Bash `run_in_background: true`；发起后不等待结果，继续本步骤后续项，结果在 Step 6 收集；`external-review` 为占位命令名）：
       ...
       external-review \
         ...
         --timeout 1800
       ```
   - [ ] 已按「外部依赖入口」加载本地 diff review skill（`~/.claude/skills/flow-code-review/SKILL.md`），以 `--json --effort <档位>` 调用
...
7. 针对 Bugs（含外部审查发现）执行 `tdd` 技能，自动推进修复，但有轮数上限
   ...
   - [ ] 自动推进全部 Bugs 修复
   - [ ] 修复后重新验证相关测试与 code-review
   - [ ] **提交收口**（Bugs 全部关闭后执行；Slice `done` 的前置条件；worktree 模式下在 `<working-dir>` 中提交）：

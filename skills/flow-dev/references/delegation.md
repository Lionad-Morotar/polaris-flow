...
并行分派的文件清单要区分「禁改」与「只读消费」：一侧的 services/工具文件对另一侧应允许调用（只读消费）——若被禁改一侧恰好需要对方的服务函数，封死调用面会逼它就地带内联请求等绕行实现，把项目分层铁律的违例带进产物（实证：并行隔离指令封死采纳服务文件的修改面，开发子代理遂在组件内联 $fetch，由 DevLoop 审查纠正）。确需双侧都修改同一文件时改为串行分派。
...
- 并行 Slice 的提交交错：多个并行子代理的提交按完成时序交错进入同一分支，审查分派的 diff 范围必须给显式区间 `git diff <本 Slice 基 ref>..<本 Slice 末提交>`，不能用 `base...HEAD` 之类吞进对侧 Slice 提交的写法；本 Slice 末提交 hash 在验收核对提交后取得

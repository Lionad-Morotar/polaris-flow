...
这些提示词配方适用于 flow-image 的 gen / edit / transparent / batch 模式。

这些可作为起点。它们故意是完整的提示词配方,而不是每个用户请求默认要添加的增强量。

调整用户提示词时:
- 保留用户提供的要求
- 仅按 `SKILL.md` 中的具体性策略添加细节
- 不要将获得下方每个示例当作可以 invent 额外故事元素的许可

带标签的行是提示词脚手架,不是封闭模式。`Asset type` 和 `Input images` 仅用于提示词,不是 API 参数。

对于透明图像,先提示生成纯平色键背景,然后用 `~/.claude/skills/flow-image/scripts/remove_chroma_key.py` 在本地移除背景;完整工作流见 `transparent.md`。

提示词原则(结构、具体性、不变量、迭代)见 `prompting.md`。
...
### editorial-poster
```
Use case: editorial-poster
Primary request: minimalist editorial vertical poster of <theme>, collectible magazine-cover quality
Style/medium: minimalist editorial layout, extreme macro photography, premium print texture
Composition/framing: vertical canvas; upper two-thirds pure negative space on a pale cool-beige or soft stone background; lower third is a macro close-up of the subject emerging from the bottom edge with only its upper part visible, pulling the eye into surface detail
Color system: pale cool-beige background; deep dominant subject color with near-black layers; translucent amber mid-transition; clear transparent and silver-grey cold highlights; no more than five color relationships, all serving one mood
Materials/textures: subject edges thin and translucent with moist reflections, elegant refraction, tiny adhering particles and visible surface tension
Lighting/mood: soft cool lighting, brighter than typical commercial ads; delicate edge highlights; soft internal gradient inside the subject
Typography: clean airy English headline with elegant letterspacing in deep charcoal, floating asymmetrically in the negative space; optional short handwritten tagline; letterforms echo the subject's edge lines
Text (verbatim): "<HEADLINE>"[, "<tagline>"]
Constraints: render headline and tagline verbatim, each exactly once; restrained uncluttered layout; no extra text; no watermarks; no unrelated logos
```
...
后处理说明:生成后运行 `python3 ~/.claude/skills/flow-image/scripts/remove_chroma_key.py --input <source> --out <final.png> --auto-key border --soft-matte --transparent-threshold 12 --opaque-threshold 220 --despill`,完整工作流见 `transparent.md`。复杂主体(头发、皮毛、玻璃、烟雾、液体、半透明材质、反射或柔和阴影)先询问用户是否继续。

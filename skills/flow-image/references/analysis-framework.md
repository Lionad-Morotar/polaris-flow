...
将图片或文本描述转换为 AI 绘画提示词时使用的分析框架与技巧。
...
**分析后构建提示词:**
1. 先发散后收敛:三层分析是全量素材库,不是提示词本身。构建时按骨架维度提取审美基因——构图逻辑、色彩系统、材质语言为通用三维,画面含文字设计时加排版骨架;每个维度只保留服务同一情绪目标的描述,与情绪无关的微细节、符号学解读与联觉留在分析里,不进入提示词
2. 骨架写法:构图写机制(主体与画框的几何关系、视线动线),不写孤立术语;色彩写关系(主色/过渡/高光/点缀,整套色系不超过五种主色关系);材质写物理属性与光学响应(边缘、反光、折射、附着物),不写"质感很好"这类空泛评价
3. 如用户指定目标风格,优先采用该风格描述
4. 生成提示词

...
2. 考校
   ...
   - [ ] 首批从简单题起步（字面/位置/行为事实），验证基础后逐批升档到机制语义/设计决策——难度阶梯见 quiz-method.md
   - [ ] 用 AskUserQuestion 逐批出题（每批 ≤4 题）：选项全部由近似变体构成，无送分项；题面指向动机与根因，不问字面事实；选项经打乱流水线呈现（起草时正确项固定 options[0] → `scripts/quiz-shuffle.mjs` 打乱并落盘答案键 → 呈现前主代理不接触顺序），防「正确项位置/长度泄漏」
...
3. 收尾
   - [ ] 终端输出简洁理解校准报告：逐题对错清单（表格）+ 理解缺口 + 真实问题清单（如有），不粘贴题面与选项全文
   - [ ] 完整存档到 `docs/quiz/YYMMDD-<slug>.md`（slug 取本次考校对象主题）：逐题记录题面、全部选项、正确答案（附 file 依据，不写行号——行号易漂移）、解析、用户作答与一句话校准结论，另附理解缺口汇总与可迁移认知；报告与存档内容一致（存档为完整版）
